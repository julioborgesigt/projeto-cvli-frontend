<template>
  <div>
    <div class="page-header">
      <h1>Gerenciamento de Status de Pedido</h1>
      <p>Adicione, edite ou remova os status de pedido cautelar do sistema.</p>
    </div>

    <div class="form-container">
      <input type="text" v-model="newItemName" placeholder="Ex: Deferido" @keyup.enter="addItem" />
      <button @click="addItem" class="btn btn-primary">Adicionar Status</button>
    </div>

    <div v-if="loading" class="loading-message">Carregando status de pedido...</div>
    ...
    <p v-else class="no-results-message">Nenhum status de pedido cadastrado.</p>
  </div>
</template>
...
const API_ENDPOINT = '/admin/status-pedido';
...
const addItem = async () => {
  ...
  try {
    ...
    newItemName.value = '';
    await fetchItems();
    notificationStore.showNotification({ message: "Status de pedido adicionado com sucesso!", type: 'success' });
  } catch (err) {
...
const saveEdit = async (id) => {
  ...
  try {
    ...
    await fetchItems();
    notificationStore.showNotification({ message: "Status de pedido atualizado com sucesso!", type: 'success' });
    cancelEdit();
  } catch (err) {
...
const deleteItem = async (id) => {
  if (confirm('Tem certeza que deseja EXCLUIR PERMANENTEMENTE este item?')) {
    try {
      ...
      await fetchItems();
      notificationStore.showNotification({ message: "Status de pedido excluído com sucesso!", type: 'success' });
    } catch (err) {
...
const toggleStatus = async (item) => {
  ...
  if (confirm(`Tem certeza que deseja ${action} este item?`)) {
    try {
      ...
      await fetchItems();
      notificationStore.showNotification({ message: `Status de pedido ${action} com sucesso.`, type: 'success' });
    } catch (err) {
