<template>
  <div>
    <div class="page-header">
      <h1>Gerenciamento de Andamentos do Procedimento</h1>
      <p>Adicione, edite ou remova os andamentos que podem ser selecionados no sistema.</p>
    </div>

    <div class="form-container">
      <input type="text" v-model="newItemName" placeholder="Ex: Relatado" @keyup.enter="addItem" />
      <button @click="addItem" class="btn btn-primary">Adicionar Andamento</button>
    </div>

    <div v-if="loading" class="loading-message">Carregando andamentos...</div>
    ...
    <p v-else class="no-results-message">Nenhum andamento cadastrado.</p>
  </div>
</template>
...
const API_ENDPOINT = '/admin/andamentos';
...
const addItem = async () => {
  ...
  try {
    ...
    newItemName.value = '';
    await fetchItems();
    notificationStore.showNotification({ message: "Andamento adicionado com sucesso!", type: 'success' });
  } catch (err) {
...
const saveEdit = async (id) => {
  ...
  try {
    ...
    await fetchItems();
    notificationStore.showNotification({ message: "Andamento atualizado com sucesso!", type: 'success' });
    cancelEdit();
  } catch (err) {
...
const deleteItem = async (id) => {
  if (confirm('Tem certeza que deseja EXCLUIR PERMANENTEMENTE este item?')) {
    try {
      ...
      await fetchItems();
      notificationStore.showNotification({ message: "Andamento excluído com sucesso!", type: 'success' });
    } catch (err) {
...
const toggleStatus = async (item) => {
  ...
  if (confirm(`Tem certeza que deseja ${action} este item?`)) {
    try {
      ...
      await fetchItems();
      notificationStore.showNotification({ message: `Andamento ${action} com sucesso.`, type: 'success' });
    } catch (err) {
